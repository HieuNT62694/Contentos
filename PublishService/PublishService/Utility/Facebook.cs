...
namespace PublishService.Utility
{
    public class Facebook
    {
        ...
        {
            _accessToken = "EAAYoOnn95VwBAMeAp76TyZCZAQSe12bQS1xvj7c1r8AFC2rN584uuxZACA4U8ZCai7gsvlyyr9I8YvdNNmRwNjvMn3zWTZAZCqtv6wlbMYiQMoek8UJU1OmLRRJ3fKDGLPFf3C7PNGKQqjaTOdCieA7U9Ts1SKHlryUHP1ly8AZBiZAdFxKJtPBY2oWZBH04ZAnaMZD";
            _pageID = "106817360743488";
        ...
        public async Task<Tuple<int, string>> PublishSimplePost(string postText)
        {
            using (var http = new HttpClient())
            {
                ...
                var httpResponse = await http.PostAsync(_postToPageURL,new FormUrlEncodedContent(postData));
        ...
        public async Task<Tuple<int, string>> UpdatePhotoWithPost(string postID, string postText)
        {
            using (var http = new HttpClient())
            {
                ...
                var httpResponse = await http.PostAsync($"{_facebookAPI}{postID}",new FormUrlEncodedContent(postData));
                var httpContent = await httpResponse.Content.ReadAsStringAsync();
