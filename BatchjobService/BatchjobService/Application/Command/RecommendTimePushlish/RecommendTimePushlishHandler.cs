...
{
    public class RecommendTimePushlishHandler : IRequestHandler<RecommendTimePushlishCommands, List<RecommendsTimeModels>>
    {
        private readonly ContentoDbContext _context;

        public RecommendTimePushlishHandler(ContentoDbContext context)
        {
            _context = context;
        }
        public async Task<List<RecommendsTimeModels>> Handle(RecommendTimePushlishCommands request, CancellationToken cancellationToken)
        {
            var retTime = new List<RecommendsTimeModels>();
            
            foreach (var item in request.IdFanpages)
            {
                var lstInterDay = _context.FanpagesInteraction.Where(x =>  x.IdFanpages == item && x.CreatedDate.GetValueOrDefault().Hour == 23 && x.CreatedDate.GetValueOrDefault().DayOfYear >= DateTime.UtcNow.AddDays(-7).DayOfYear && x.CreatedDate.GetValueOrDefault().DayOfYear < DateTime.UtcNow.DayOfYear).OrderByDescending(x=>x.Interaction).FirstOrDefault();
