...
namespace ContentProccessService.Application.Queries.GetContentsByWriterId
{
    public class GetContentByWriterIdHandler : IRequestHandler<GetContentsByWriterIdRequest, List<ContentsViewModel>>
    {
        ...
        public GetContentByWriterIdHandler(ContentoDbContext context)
        {
        ...
        {
            var list =  await context.Contents.AsNoTracking().Include(c => c.IdTaskNavigation).Where(t => t.IdTaskNavigation.IdWritter == request.IdWriter
            ...
            foreach (var item in list)
            {
                ...
                {
                    Id = item.Id,
                    //Comment = item.Comments.FirstOrDefault(item.id),
